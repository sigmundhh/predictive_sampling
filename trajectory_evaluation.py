...
class TrajectoryEvaluator:
    ...
    def evaluate_trajectories(self, state_trajs, control_trajs):
        ...
        for i in range(state_trajs.shape[0]):
            costs[i] = self.total_trajectory_cost(state_trajs[i,:,:], control_trajs[i,:])
...
if __name__ == "__main__":
    ...
    zeros = np.zeros((4, 4))
    us = np.array([0, 0, 0, 0])
    print(evaluator.total_trajectory_cost(ones, us)) # this should be lower than the next
    zeros[-1] = ones[-1]
